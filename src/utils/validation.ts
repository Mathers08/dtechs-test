...
export const validationUniqueUsername = (users: IUser[], values: IUser) => {
  ...
  if (usernames.includes(currentUser?.username)
    && window.location.pathname !== `/users/${currentUser?.id}`) {
...
export const validationSchema = yup.object().shape({
  username: yup.string().min(3, 'Поле username должно иметь не менее 3 символов')
    .required('Поле обязательное'),
  password: yup.string().min(4, 'Поле password должно иметь не менее 4 символов')
    .required('Поле обязательное'),
  firstName: yup.string().min(2, 'Поле firstName должно иметь не менее 2 символов')
    .required('Поле обязательное'),
